...
MODELO_MOT="MOT17-02-DPM"
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
IMAGES_DIR = f"/home/jetson/Documents/MOT17/train/{MODELO_MOT}/img1"
CSV_RENDIMIENTO = os.path.join(os.getcwd(), "rendimiento_tiempo_relativo.csv")
DETECTIONS_OUTPUT_FILE = os.path.join(os.getcwd(), MODELO_MOT+".txt")
...
with open(CSV_RENDIMIENTO, mode='a', newline='') as f_rendimiento, \
    ...
    for path in imagenes:
        ...
        print(f"✅ {nombre} procesada. Tiempo: {tiempo_relativo:.2f}s |  Detecciones: {len(results[0].boxes) if results and results[0].boxes else 0}")
